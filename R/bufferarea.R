bufferArea <- function(df) {
  ...
  pts_sf <- st_transform(pts_sf, crs=32616)
  ...
  # Import boundary
  chib <- st_transform(chib, crs=32616)

  # Perform intersection
  pts_pol_int <- st_intersection(pts_pol,chib)

  # Calculate areas
  stationarea <- st_area(pts_pol_int)
  divvyarea <- sum(stationarea)
  cityarea <- st_area(chib)
